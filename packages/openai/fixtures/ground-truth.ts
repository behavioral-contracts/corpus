...
export async function embeddingsCreateNoCatch(text: string) {
  // SHOULD_FIRE: authentication-error — embeddings.create throws, no try-catch
...
export async function audioTranscriptionNoCatch(audioFile: File) {
  // SHOULD_FIRE: authentication-error — audio.transcriptions.create throws, no try-catch
...
export async function moderationsCreateNoCatch(text: string) {
  // SHOULD_FIRE: authentication-error — moderations.create throws, no try-catch
...
export async function fineTuningJobNoCatch(trainingFileId: string) {
  // SHOULD_FIRE: authentication-error — fineTuning.jobs.create throws, no try-catch
...
// ─────────────────────────────────────────────────────────────────────────────
// 9. Multiple calls — each fires independently
// ─────────────────────────────────────────────────────────────────────────────

export async function multipleCallsNoCatch(text: string) {
  // SHOULD_FIRE: authentication-error — first call, no try-catch
  const embedding = await openai.embeddings.create({
    model: 'text-embedding-ada-002',
    input: text,
  });
  // SHOULD_FIRE: authentication-error — second call, no try-catch
